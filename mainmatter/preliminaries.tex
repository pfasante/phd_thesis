...
\end{proof}
%The complement of a direct sum is the direct sum of its complements, \ie/
%\begin{equation*}
%    \parens{V_1 \times \cdots \times V_n}^\perp = V_1^\perp \times \cdots \times V_n^\perp,
%\end{equation*}
%Given two subspaces $U,V$ it holds that
%\begin{equation*}
%    U \subseteq V \Rightarrow V^\perp \subseteq U^\perp.
%\end{equation*}
%Some well known properties of complements are the following.
...
\marginpar{%
    ...
    \begin{tikzpicture}[node distance=2cm]
        ...
        \path [draw, ->] (ul) edge node[above] {$\cdot \alpha$} (ur);
        \path [draw, ->] (ul) edge node[left] {$\Phi$} (ll);
        \path [draw, ->] (ll) edge node[below] {$T_{\alpha}$} (lr);
        \path [draw, ->] (lr) edge node[right] {$\Phi^{-1}$} (ur);
    \end{tikzpicture}
...
\marginpar{%
    \footnotesize
    %\vspace*{-29em}
    \textbf{An unbounded adversary breaks\\computational indistinguisability}\\[2pt]
...
Given several blocks of a plaintext message $m_1, m_2, \ldots \in \F_2^n$ and a block cipher $E : \F_2^n \to \F_2^n$.
...
\marginpar{%
    ...
    \begin{tikzpicture}
        ...
        \node (Ek) [below=2em of Ek_in_xor,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=1cm] {$E_k$};
        \node (Ek_out)[below=2em of Ek] {$c_i$};
        \path[line] (p) edge (Ek_in_xor);
        \path[line] (iv) edge (Ek_in_xor);
        \path[line] (Ek_in_xor) edge (Ek);
        \path[line] (Ek) edge (Ek_out);
        \path[line] (Ek.south) -- +(0,-7.5pt) -- +(25pt,-7.5pt) -- +(25pt,55.5pt) -- +(+40pt,55.5pt);
    \end{tikzpicture}
...
\marginpar{%
    ...
    \captionof{figure}[Modes of Operation]{Block Cipher Modes of Operation: ECB (a), CBC (b), and CTR (c)}\label{fig:modes}
}
%\marginpar{%
%    \centering
%    \vspace*{-26em}
...
%        \path[line] (Ek.south) -- +(0,-7.5pt) -- +(25pt,-7.5pt) -- +(25pt,55.5pt) -- +(+40pt,55.5pt);
%    \end{tikzpicture}
%    \captionof{figure}{CBC Mode of Operation}\label{fig:modes-cbc}
%}
%\marginpar{%
%    \centering
%    \vspace*{-10em}
...
\begin{definition}[Product cipher]\label{def:product-cipher}
    Given a block cipher $E : \F_2^n \to \F_2^n$, with keys $k = (k_1, \ldots, k_r) \in \F_2^{rm}$.
    ...
    \end{equation*}
    where $R_{i,k_i}$ permutes the $\F_2^n$.
...
\marginpar{%
    ...
    \begin{tikzpicture}
        ...
        \node (p) {$m$};
        \node (f-1) [below=1em of p,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=1cm] {$R_{1}$};
        \path[line] (p) edge (f-1);

        \node (dots)[below=0.5em of f-1,node distance=1.5cm] {$\vdots$};
        \path[line] (f-1) edge ($(dots.north)+(0,-0.5em)$) ;

        \node (f-r) [below=1em of dots,node distance=1.5cm,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=1cm] {$R_{r}$};
        \path[line] (dots) edge (f-r) ;

        \node [below = 1em of f-r] (c) {$c$};
...
\marginpar{%
    ...
    \begin{tikzpicture}
        ...
        \node [above=1em of XOR-1] (p) {$m$};
        \node (fs-1) [below=1em of XOR-1,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=0.5cm] {$S$};
        \node (fl-1) [below=-0.5pt of fs-1,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=0.5cm] {$L$};
        \path[line] (p) edge (XOR-1);
        \path[line] (XOR-1) edge (fs-1);

        \node (XOR-2)[below=1em of fl-1,XOR,scale=1.2] {};
        \path[line] (fl-1) edge node[right] {} (XOR-2);
        \node (dots)[below=0.5em of XOR-2] {$\vdots$};
        \path[line] (XOR-2) edge ($(dots.north)+(0,-0.5em)$) ;

        \node (XOR-3)[below=1em of dots,XOR,scale=1.2,node distance=1.5cm] {};
        \path[line] (dots) edge (XOR-3) ;
        \node (fs-3) [below=1em of XOR-3,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=0.5cm] {$S$};
        \node (fl-3) [below=-0.5pt of fs-3,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=0.5cm] {$L$};
        \path[line] (XOR-3) edge (fs-3);

        \node (XOR-4)[below=1em of fl-3,XOR,scale=1.2] {};
        \path[line] (fl-3) edge node[below] {} (XOR-4);
        \node [below = 1em of XOR-4] (c) {$c$};
...
\marginpar{%
    ...
    \begin{tikzpicture}
        \node (in_l) {$x_i$};
        \node (in_r) [right = 6em of in_l] {$y_i$};
        \node (xor) [below=4em of in_l,XOR,scale=1.2,node distance=1.5cm] {};
        \node (f) [right = 2em of xor,draw,rectangle,thick,rounded corners,minimum width=1cm,minimum height=1cm] {$G_i$};
        \node (k) [above = 1em of f] {$k_i$};
        \node (swap_l) [below = 2em of xor] {\hphantom{$x_i$}};
        \node (swap_r) [right = 6em of swap_l] {\hphantom{$y_i$}};
        \node (out_l) [below = 3em of swap_l] {$x_{i+1}$};
        \node (out_r) [below = 3em of swap_r] {$y_{i+1}$};
...
A different approach is the notion of concrete security, developed by \textcite{C:BelKilRog94} and applied to assess the security of typical modes of operations~\cite{FOCS:BDJR97}.
...
%The goal of the attacker is to tell apart those two cases, \ie/ distinguish them.
%This distinguishing probability is the attackers \emph{advantage}; for a block cipher $E$ and adversary $A$ making at most $q$ queries, we denote it $\mathrm{Adv}_{E,A}(q)$.
...
We again take a top-down approach that is comprehended in the margin note.
\marginpar{%
    \footnotesize
    ...
    \begin{align*}
        ...
        \intertext{2.\ Differentials consists of many trails}
        ...
        \intertext{6.\ Maximum EDP for \SPN/ is bounded by}
        &\hphantom{=}\ \mathrm{MEDP}(E) \leqslant p_S^{\text{\#active S-boxes}}
    \end{align*}
}
...
\end{equation}
Notably exceptions are~\citeonly{FSE:HLLSCC00,rijndael_book,FSE:PSLL03,EC:CanRou15}, that spend effort to prove bounds on the \MEDP/ for two rounds of some key-alternating ciphers.
...
\marginpar{%
    ...
    \begin{gather*}
        ...
        2^{n+1}\bias^L_{E_k}(\alpha, \beta) = 2^n \cor_{E_k}(\alpha, \beta) = \FC{E_k}(\alpha, \beta)
        \intertext{2.\ Linear hull over composition $F = H \circ G$}
        ...
        \mathrm{MELP}(E) = \max_{\alpha, \beta \in \F_2^n\setminus\set{0}} \frac{1}{2^m} \sum_{k \in \F_2^m} {\cor_{E_k}(\alpha, \beta)}^2
...
A common approach is, to define a linear trail corresponding to the differential trail as a linear approximation over each round, specifying all the intermediate masks.
...
\citeauthor{PhD:Daemen95}'s main idea to achieve a high number of active S-boxes is to split the linear layer in two parts, a mixing and a transposition step.
...
They where studied in \eg/~\citeonly{EC:Evertse87,FSE:Lai94,TIT:CanVid05}, and quite recently appeared in the context of invariant attacks, see~\cite{C:BCLR17}.
...
\Cref{def:bias-cor-walsh} gave the definition of the Walsh transform for vectorial Boolean functions only.
