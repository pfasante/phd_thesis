...
\marginpar{\vspace{-10pt}\includegraphics[width=50mm]{bison/bison-logo}\\\footnotesize\hphantom{.}\hfill---Lucas Hartmann}
...
%We briefly recall the Whitened Swap-or-Not construction, recapitulate properties of Boolean functions and shortly cover differential and linear cryptanalysis.
%We denote by $\F_2$ the finite field with two elements and by $\F_2^n$ the $n$-dimensional vector space over $\F_2$, \ie/ the set of all $n$-bit vectors with a bitwise xor as the addition.

\section{Introduction}

%Block ciphers are among the most important cryptographic primitives as they are at the core responsible for a large fraction of all our data that is encrypted.
%Depending on the mode of operation (or used construction), a block cipher can be turned into an encryption function, a hash-function, a message authentication code or an authenticated encryption function.
%
%Due to their importance, it is not surprising that block ciphers are also among the best understood primitives.
%In particular the \AES/~\cite{FIPS:AES} has been scrutinised by cryptanalysts ever since its development in 1998~\cite{CARDIS:DaeRij98} without any significant security threat discovered for the full cipher (see \eg/~\citeonly{AES:Gilbert00,FSE:FKLSSWW00,C:BirKhoNik09,AC:BirKho09,AC:DunKelSha10,EC:DerFouJea13,ToSC:GraRecRon16,EC:GraRecRon17,AC:RonBarHel17}).
%
%The overall structure of \AES/, being built on several (round)-permutations interleaved with a (binary) addition of round keys is often referred to as key-alternating cipher and is depicted in \cref{fig:key_alt}.
%
...
%Nowadays many secure ciphers follow this construction.
%
%Interestingly, besides its overwhelming use in practice and the intense cryptanalytic efforts spent to understand its practical security, the generic (or idealised) security of key-alternating ciphers has not been investigated until 2012.
%Here, generic or idealised security refers to the setting where the round functions $R_i$ are modeled as random permutations.
%An (computational unbounded) attacker is given access to those round functions via oracle queries and additional oracle access to either the block cipher or a random permutation.
%The goal of the attacker is to tell apart those two cases.
...
%Indeed, the Even-Mansour cipher~\cite{JC:EveMan97} can be seen as a one-round version of the key-alternating cipher where the round function is a random permutation.
%
...
%Since then, quite a lot of follow-up papers, \eg/~\citeonly{C:ABDMS13,TCC:GuoLin15,FSE:LamSeu14,C:HoaTes16}, managed to improve and generalise this initial result significantly.
%In particular,~\cite{EC:CheSte14} managed to give a tight security bound for any number of rounds.
%Informally, for breaking the $r$-round Even-Mansour construction, any attacker needs to make roughly $2^{\frac{r}{r+1}n}$ oracle queries.
%
%While this bound can be proven tight for the iterated Even-Mansour construction, it is unsatisfactory for two reasons.
...
%Motivated by this theoretical defect and the importance of encrypting small domains with full security (see \eg/~\cite{EPRINT:MirYil17}), researchers started to investigate alternative ways to construct block ciphers with the highest possible security level under minimal assumptions in ideal models.
...
%In the \cref{bison:sec:implementation} we report implementation figures for \bison/.
%Those figures underline our believe that it will be very hard to design a concrete instance that has a performance comparable to key-alternating ciphers.
%However, based on our implementation figures, one important metric where an instance of the \WSN/ construction might come close to being competitive is the code size.
%Moreover, asymptotically, for the encryption of an $n$-bit block, \bison/ requires $\mathcal{O}(n^2)$ bit operations.
%Designing a scalable family of ciphers with asymptotically significantly less operations seems challenging.
            ...
            \begin{itemize}
                ...
                \item $f_{b(i)}$ is defined as
                        \begin{align*}
                            ...
                            f_{b(i)}(x) &\coloneqq \angles{x[1 : (n-1)/2],x[(n+1)/2 : n-1]} + b(i),
                        \end{align*}
...
Regarding cycles per byte, this is three orders of magnitude slower than optimised implementations of \AES/\@.
        ...
        \begin{minipage}{0.90\textwidth}
            ...
            \begin{itemize}
                ...
                \item $f_{b(i)}$ is defined as
                    ...
                    \end{align*}
                \item $g_0$ is defined by its truth table in hexadecimal form: $\mathtt{00071356}$, and $g_1$ as $g_0 + 1$,
                \item $h$ is defined as
                        \begin{align*}
                            h : \F_2^{n-6} &\to \F_2 \\
                            h(x_1, \ldots, x_{n-6}) &\coloneqq \angles{x[1 : n/2-3], x[n/2-2 : n-6]}\;.
                        \end{align*}
                \item and $b(i)$ is $0$ if $i \leqslant \frac{r}{2}$ and $1$ otherwise.
            \end{itemize}
            \vspace{-0.25\baselineskip}
...
% Our instance is restricted to odd block lengths, while for efficiency reasons often even block lengths are chosen.
% Circumventing this constraint while retaining our strong security arguments appears to be an interesting challenge.\ac{I don't like this remark. The analysis also works when $n$ is even. E.g. we could use $f(x)=(x_1,\ldots, x_{n/2-1}) \cdot (x_{n/2},\ldots, x_{n-2}) + x_{n-1}$?}
...
%From a more general perspective, what we do often is to come up with more and more efficient schemes, but the arguments we give for security do not really get better.
%Compare that how provable security works:
%There people try first to come up with some scheme, not taking into account efficiency too much, that is secure in some model and in a second step try to find schemes that improve.

%Finally, The design of block ciphers has evolved from an art to a science in the recent 40 years.
%Today we are not only able to design very efficient ciphers, but also ones we firmly believe to be secure -- with the \AES/ being the most prominent example withstanding 20 years of cryptanalysis.
%While efficiency bounds have been pushed further and further within the trend of lightweight cryptography, the time may has come, where we should shift our focus to improving security arguments for new designs -- similar to the area of %provable security.
%In that area, efficiency is only taken into account as the second step, to improve schemes proven to be secure in some model, but which are initially often totally impractical.
