...
\begin{algorithmic}[1]
    ...
    \Function{Counting Key Recovery}{$F$, $\propDiff{\alpha}{F^r_k}{}{\beta}$, $K$, $S$}
        ...
        \ForAll{$k^\prime \in K$}
            ...
            \EndFor{}
        \EndFor{}
    \State{}\Return{key candidate $k$ such that $\forall k_\prime \in K : \mathrm{cnt}_k \geqslant \mathrm{cnt}_{k^\prime}$}
    \EndFunction{}
\end{algorithmic}
\end{algorithm}
...
\textcite{RSA:LKKD08} proposed the \emph{early abort} technique which is based on the simple idea to find conditions which can be checked during partial en- or decryption of plaintext/ciphertext pairs, based on a subset of the guessed key-bits.
...
\newthoughtpar{Switching to affine subspace}
...
\begin{algorithmic}[1]
    ...
    \Ensure{A truncated differential $\propDiff{\coset{U}{a}}{F}{}{\propDiff{\cdots}{F}{}{\coset{V}{b}}}$.}
    \Statex{}
...
%This deterministic propagation is due to the following lemma.
%\begin{lemma}\label{lem:ui-subspace-of-vi}
%    Given $U_i$, $U_{i+1}$ and $V_i$ with $\propDiff{U_i}{F}{}{U_{i+1}}$ and $\propDiff{U_{i+1}}{F^{-1}}{}{V_i}$.
%    Then
%    \begin{equation*}
%        U_i \subseteq V_i.
%    \end{equation*}
%\end{lemma}
%\begin{proof}
%    $U_{i+1}$ is defined as:
%    \begin{equation*}
%        U_{i+1} = \Span{F(x) \given x \in U_{i}},
%    \end{equation*}
%    and $V_{i}$ as
%    \begin{equation*}
%        V_{i} = \Span{F^{-1}(y) \given y \in U_{i+1}}.
%    \end{equation*}
%    From the definition of a linear span, we have that $\set{F(x), x \in U_{i}} \subseteq U_{i+1}$ and consequently $\set{F^{-1}(z), z \in \set{F^{-1}(x), x \in U_{i+1}}} \subseteq V_{i+1}$.
%
%    It follows that $ U_{i+1} = \Span{x \in U_{i+1}} \subseteq V_{i+1}$ which proves the lemma.
%\end{proof}
...
\begin{proof}
    ...
    \end{equation*}
    %, which proves that $0$ is present in every $I(\alpha)$.
    % cf cancellation in $ F(x+\delta_x, k+\delta_k)+F(x+\delta_x, k)+F(x, k+\delta_k)+F(x, k)$ when fixing \delta_k to 0
...
Let us conclude with an example comparing linear and affine subpsace propagations.
...
%\subsection{Turning the output of our algorithm into a key recovery attack}
%
%Assume we have a differential distinguisher $\alpha \to \beta$ for $r$ rounds of some cipher.
%We use our algorithm to get a key recovery strategy that extends this distinguisher on the ciphertext side by $s$ rounds.
%Let us discuss, how an $r+s$ key recovery attack on this cipher is mounted.
%This is depicted in \cref{fig:key-rec:ext_ciphertext_side}.
%
%\begin{figure}[t]
%    \centering
%    \missingfigure{ciphertext side extension}
%    \caption{%
%        $s$ round extension of an $r$ round distinguisher on the ciphertext side.
%    }\label{fig:key-rec:ext_ciphertext_side}
%\end{figure}
%
%The attacks aim is, to recover all key bits that influence the distinguisher.
%In particular these are all bits that the output difference $\beta$ depends on.
%Thus, as usual, we start with requesting the $r+s$ round encryption of plaintext pairs $X_i = (x_i, x_i^\prime = x_i + \alpha)$.
%Denote these encryptions $Y_i = (y_i, y_i^\prime)$.
%
%During the key guessing step, we (partially) decrypt these $Y_i$ pairs to eventually check, if they fulfill the distinguisher's output difference $\beta$.
%In order to reduce the data complexity/making the distinguishing procedure more effective, we apply the early abort technique.
%We basically want to know, for each ciphertext pair $Y_i$ and every possible key candidate $k^\prime$, if this $Y_i$ is filtered or not.
%Using the information from our obtained attack strategy, this works as follows.
%
%\begin{enumerate}
%    \item Filter out all pairs $Y_i$ whose difference is not in $U_s$.
%    \item Follow the strategy found by the algorithm, that is an order of conditions to check, linked with key bits to guess, $(c_i, [k_{i,j}]_{j=1}^m)_{i=1}^\ell$.
%    \item For every pair $Y_i$:
%    \begin{enumerate}
%        \item For every condition $c_i$:
%        \begin{enumerate}
%            \item Guess the corresponding key bits $[k_{i,j}]$.
%            \item Partially decrypt $(y_i, y_i^\prime)$ up to the point where the condition has to be checked.
%            \item Compute the corresponding difference $\delta_i$ and check if $\iprod{\delta_i}{c_i} = 0$ (that is, if the condition is fulfilled).
%        \end{enumerate}
%        \item For every key candidate, where the condition is \emph{not} fulfilled, the current pair $Y_i$ is filtered and ignored for verifying the distinguisher.
%    \end{enumerate}
%\end{enumerate}
%
%Eventually we should end up with something like \cref{tab:key_rec_result}.
%In this case, for key candidate $k_2$ one pair $Y_i$, and for key candidate $k_{n-1}$ two pairs survive the filtering and fulfill the distinguisher $\alpha \to \beta$.
%We thus assume that most probable $k_{n-1}$ or maybe $k_2$ is the correct key guess.
%
%\begin{table}
%    \centering
%    \caption{%
%        Result of running the key recovery attack.
%        Assume we have $n$ key candidates.
%    }\label{tab:key_rec_result}
%    \begin{tabular}{cc}
%        \toprule
%        Key Candidate & Number of pairs that fulfill the distinguisher \\
%        \midrule
%        $k_0$         & 0 \\
%        $k_1$         & 0 \\
%        $k_2$         & 1 \\
